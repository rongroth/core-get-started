// https://github.com/NithinBiliya/arboreal
...
(function () {
   ...
   *  result - result list
   *
   * detials - will populate result with list of all nodes at requested level from the current level
   */
  Arboreal.prototype.drill = function (node, clevel, rlevel, result) {
    (function walkDown (node, clevel, rlevel, result) {
      var i;
      if (clevel == rlevel)
        ...
        for (i = 0; i < node.children.length; i++) {
          if(node.children[i]!==null) {
            walkDown(node.children[i], clevel+1, rlevel, result);
          }
        }
      }
